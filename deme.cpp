...
void Deme::compute_next_generation() {
    ...
    for (int i = 0; i < trunc(pop_.size() / 2); i++) {
        ...
        int rand_a = (generator_() % 100) + 1;//1-100
        generator_.seed(chrono::system_clock::now().time_since_epoch().count());
        int rand_b = (generator_() % 100) + 1;//1-100
...
// if fitnesses are 60 for a and 40 for b, there should be a 60% chance of a and 40% chance of b, etc
